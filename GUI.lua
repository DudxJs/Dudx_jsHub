...
-- Variável global para armazenar a lista suspensa visível
local activeDropdownList = nil

-- Utilitário para fechar qualquer lista suspensa visível
local function closeActiveDropdown()
    if activeDropdownList and activeDropdownList.Parent then
...
function DudxJsGUI:New(title, toggleImageId, creatorName)
    ...
    function self:ToggleGUI()
        -- Adiciona o fechamento de segurança antes de alternar
        closeActiveDropdown() 
    ...
    TopBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            -- Fechamento de segurança ao arrastar a GUI
            closeActiveDropdown()
            dragging = true
    ...
    Title.Text = title or "Dudx_jsGUI"
    ...
    CreatorCredit.Text = "By " .. (creatorName or "Dudx_js") -- Usa o parâmetro creatorName ou um padrão
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        -- Fechamento de segurança antes de mostrar o quadro de confirmação
        closeActiveDropdown()
    ...
    MinBtn.MouseButton1Click:Connect(function()
        ...
        -- Fechamento de segurança ao minimizar
        closeActiveDropdown()
...
function DudxJsGUI:AddTab(tabName)
    ...
    -- Novo: Adiciona um listener para fechar a lista suspensa ao rolar o scroll
    contentScroll.Changed:Connect(function(property)
    ...
    button.MouseButton1Click:Connect(function()
        -- Fechamento de segurança ao alternar de aba
        closeActiveDropdown() 
    ...
    function tab:AddButton(text, callback)
        ...
        btn.MouseButton1Click:Connect(function()
            -- Fechamento de segurança antes de executar o callback
            closeActiveDropdown() 
    ...
    function tab:AddDropdown(text, items, callback)
        ...
        listFrame.ZIndex = 50
        listFrame.Size = UDim2.new(0, dropdownContainer.Size.X.Offset, 0, 150) -- Novo: Tamanho X da lista igual ao do container
        ...
        local function atualizarLista()
            ...
            for idx, nome in ipairs(items) do
                ...
                item.MouseButton1Click:Connect(function()
                    ...
                    closeActiveDropdown() -- Fecha a lista após a seleção
        ...
        dropdownContainer.MouseButton1Click:Connect(function()
            ...
            -- Novo: Fecha qualquer outra lista suspensa antes de abrir uma nova
            closeActiveDropdown()

            -- Garante que a lista esteja atualizada
            atualizarLista() 

            -- Calcula a posição absoluta na tela do dropdownContainer
            local absPos = dropdownContainer.AbsolutePosition
            local absSize = dropdownContainer.AbsoluteSize

            -- Define a posição da lista suspensa para que ela apareça logo abaixo do botão
            listFrame.Position = UDim2.new(
                0, absPos.X, 
                0, absPos.Y + absSize.Y
            )
            
            -- Muda o pai da lista temporariamente para a ScreenGui, para que a posição absoluta funcione
            listFrame.Parent = gui 
            listFrame.Visible = true

            -- Novo: Armazena a lista como a lista suspensa ativa
    ...
    function tab:AddLabel(text)
        ...
        label.Size = UDim2.new(1, 0, 0, 28) -- AUMENTADO DE 24 PARA 28
        label.LayoutOrder = tab._order
        label.BackgroundTransparency = 1
        label.Text = text
        label.TextColor3 = Color3.new(1, 1, 1)
        label.Font = Enum.Font.SourceSans
        label.TextSize = 18 -- AUMENTADO DE 16 PARA 18
        label.TextXAlignment = Enum.TextXAlignment.Left
    ...
    function tab:AddCreatorProfile(userId, nickname, message) -- ALTERADO photoId PARA userId
        ...
        -- *** MUDANÇA AQUI: Busca a imagem do avatar pelo UserId ***
        ...
        if isReady then
            ...
            -- Fallback ou placeholder se a imagem não estiver pronta
            profileImage.Image = "rbxassetid://4691459461" -- Exemplo de imagem de placeholder (Roblox logo)
            warn("Imagem de perfil para UserId " .. userId .. " não pôde ser carregada imediatamente.")
        end
    ...
    function tab:AddSocialCredit(platformName, platformPhotoId, username, message, profileLink)
        ...
        platformImage.Image = "rbxassetid://" .. tostring(platformPhotoId) -- Continua usando rbxassetid
...
function DudxJsGUI:Destroy()
    -- Fechamento de segurança no método Destroy
    closeActiveDropdown() 
    if self._gui then
        self._gui:Destroy()
        self._gui = nil
