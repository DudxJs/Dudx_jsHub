    ...
    function tab:AddDropdown(text, items, callback)
        ...
        local function atualizarLista()
            ...
            for idx, nome in ipairs(items) do
                ...
                item.MouseButton1Click:Connect(function()
                    ...
                    hideAllDropdowns() -- Fecha a lista ao clicar em um item
        ...
        dropdownContainer.MouseButton1Click:Connect(function()
            if listFrame.Visible then
                hideAllDropdowns() -- Fecha se estiver visível
                return
            end
            
            hideAllDropdowns() -- FECHA QUALQUER OUTRA LISTA ANTES DE ABRIR ESTA

            -- Calcula a posição absoluta do botão de dropdown na tela
            local absolutePos = dropdownContainer.AbsolutePosition
            local absoluteSize = dropdownContainer.AbsoluteSize
            
            listFrame.Parent = gui -- Parenta para o ScreenGui para garantir visibilidade
            listFrame.Size = UDim2.new(listFrame.Size.X.Scale, dropdownContainer.AbsoluteSize.X, listFrame.Size.Y.Scale, listFrame.Size.Y.Offset)
            listFrame.Position = UDim2.new(0, absolutePos.X, 0, absolutePos.Y + absoluteSize.Y)

            atualizarLista() -- Garante que a lista esteja atualizada antes de mostrar
            listFrame.Visible = true
            activeDropdownList = listFrame -- Define a lista como a ativa
        end)

        -- Observa mudanças de visibilidade ou parent do dropdownContainer para fechar a lista
        dropdownContainer.Changed:Connect(function(prop)
            if prop == "Parent" or prop == "Visible" then
                if not dropdownContainer.Visible or not dropdownContainer.Parent then
                    hideAllDropdowns()
                end
            end
        end)
        
        -- Atualiza a posição da lista em tempo real se a GUI for movida
        ...
        tab._order = tab._order + 1
        return dropdownContainer, selectedLabel
    end
